...
class Server:
    ...
    def check_user_data(self, client_socket: socket.socket):
        ...
        try:
            ...
            print(f"Socket error in client_check with socket {client_socket.getpeername()}")
        # time.sleep(1)
    ...
    def main_message_handler(self, main_client_socket: socket.socket):
        ...
        """
        print(f'main client {main_client_socket.getpeername()} connected')
        self.main_client = main_client_socket
        self.main_client_available_status = True
        while True:
            try:
                if online_check(main_client_socket):
                    if online_check(self.client):
                        print(f'client {self.client.getpeername()} is online')
    ...
    def current_client_message_handler(self, client_socket: socket.socket):
        ...
        """
        print(f'current client {client_socket.getpeername()} connected')
        self.client = client_socket
        while True:
            try:
                ...
                break
            time.sleep(1)
        print('current client has disconnected')
    ...
    def waiting_client(self, waiting_client_socket: socket.socket):
        ...
        """
        print(f'waiting client {waiting_client_socket.getpeername()} connected')
        while True:
            try:
                if online_check(waiting_client_socket):
                    if online_check(self.main_client):
                        if self.main_client_available_status:
                            ...
                            threading.Thread(target=self.current_client_message_handler, args=(waiting_client_socket,)).start()
                            break
                        else:
                            time.sleep(10)
                            waiting_client_socket.send(
                ...
                break
            time.sleep(1)
        print(f'waiting client disconnected or transferred')
